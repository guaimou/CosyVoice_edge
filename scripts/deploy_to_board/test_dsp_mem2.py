"""Test TWO DSP DLCs init before LLM load."""
import os, sys
...
# 1. Init BOTH INT8 DSP DLCs FIRST (keep alive simultaneously)
sessions = []
for name in ["hift_f0_predictor_int8.dlc", "hift_decode_pre_istft_int8.dlc"]:
    ...
    if ret != 0:
        ...
        sys.exit(1)
    sessions.append(api)
    print(f"  {name}: kept alive")

print(f"Both DSP DLCs alive ({len(sessions)} sessions)")

# 2. Load LLM
print("Loading CosyVoice...")
from cosyvoice.cli.cosyvoice import CosyVoice
model = CosyVoice(os.path.join(PROJECT_ROOT, "pretrained/CosyVoice-300M"))
print("LLM loaded OK after 2x DSP init")
for api in sessions:
    api.Release()
print("All released")
